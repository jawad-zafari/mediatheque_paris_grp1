...
function admin_media_save($id = null)
{
    ...
    if ($type_db === 'book') {
        ...
        $duration = $extra_fields['duration_m'] ?? 0;
        ...
        $platform = $extra_fields['platform'] ?? '';
        ...
        if ($existing_media) {
            $existing_platform = $existing_media['platform'] ?? ($existing_media['plateform'] ?? '');
